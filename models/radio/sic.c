...
// <-RF00000000-AdamXu-2018/04/25-add log for radio
        PRINT_RADIO("Half1d B: radio-rx0 %"PRId64" %d, packet->rxdBm=%f, nodedata->rxdBm=%f, packet->id=%d\n", get_time(), c->node, packet->rxdBm, nodedata->rxdBm, packet->id);
// ->RF00000000-AdamXu
        ...
        nodedata->rxdBm += packet->rxdBm;
	{
		...
		while(count < nodedata->sic_iteration_limit)
		{
			...
			PRINT_RADIO("Half1d cs: nodedata->rx_busy[%d]=%d\n", count, nodedata->rx_busy[count]);
			count++;
		}
	}
// <-RF00000000-AdamXu-2018/04/25-add log for radio
        PRINT_RADIO("Half1d E: radio-rx0 %"PRId64" %d, packet->rxdBm=%f, nodedata->rxdBm=%f, packet->id=%d\n", get_time(), c->node, packet->rxdBm, nodedata->rxdBm, packet->id);
// ->RF00000000-AdamXu
        /* log cs */
